...
Male.mean.age.mort <- Male.Mortality.Table[round(mean(member.base[,2]),digits = 0) - 15 ,]
Female.mean.age.mort <- Female.Mortality.Table[round(mean(member.base[,2]),digits = 0) - 15 ,]

plot(x = seq_along(Male.mean.age.mort[1,-1]), y = Male.mean.age.mort[1,-1],
     main = paste(c("Extract of mortality rates of age ",round(mean(member.base[,2]),digits = 0), " (forecasting starts at t = 15)"))  ,
