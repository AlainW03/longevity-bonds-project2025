...
# Target Parameter: Prob of ruin 50%

EPV.mort.risk.margin <-  10 + i*5 
...
# Warning, hardcoding involved!
{
  ...
  results2 <- rbind(results[1,],(results[2,] - 50)/50)
  
  first_negative_position <- as.numeric(which(results2[2,] < 0)[1])
  
  EPV.mort.risk.margin <- as.numeric(results2[1,first_negative_position - 1]) - 0.5
  ...
  
  {
    ...
    for(i in 1:tests){
      ...
      EPV.mort.risk.margin <-  EPV.mort.risk.margin + i/2 
  ...
  results3 <- rbind(results[1,],(results[2,] - 50)/50)
  
  first_negative_position <- as.numeric(which(results3[2,] < 0)[1])
  
  EPV.mort.risk.margin <- as.numeric(results3[1,first_negative_position - 1]) - 0.05
  ...
  
  {
    ...
    for(i in 1:tests){
      ...
      simulations <- 500 #  integer at least 1
      inital.members <- 100 #  integer at least 10
      Bond.Proportion <- 0 # value between 0 to 100
      improv.factor <- 0 # value between -100 to +100
      coupon.rate <- 12.5 # value between 0 to 100
      ...
      EPV.mort.risk.margin <-  EPV.mort.risk.margin + i/20 
